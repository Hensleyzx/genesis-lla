...
export function gerarRelatorioTexto(r) {
  const lines = [
    ...
    `Leucócitos/WBC informado: ${r.leucocitos ?? '—'} ${r.unidadeLeucocitos ? `(${r.unidadeLeucocitos})` : ''}`,
    `Escala de expressão informada: ${r.escalaExpressao || '—'}`,
    `BCR-ABL1 informado: ${r.fusoes?.['BCR-ABL1'] === 'positivo' ? 'Positivo' : r.fusoes?.['BCR-ABL1'] === 'negativo' ? 'Negativo' : '—'}`,
    `Médico responsável: ${r.profissional?.name || '—'}${r.profissional?.identifier ? ` (${r.profissional.identifier})` : ''}${r.profissional?.institution ? ` — ${r.profissional.institution}` : ''}`, 
