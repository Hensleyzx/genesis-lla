...
/* ---- álgebra linear densa ---- */
  ...
  export function coxph(rows, covNames) {
  ...
  export function univariate(time, event, genes, exprRows, options = {}) {
    const { minN = 20, minEvents = 5, requireConverged = true } = options;
    const out = [];
    for (const gene of genes) {
      ...
      const nEvents = rows.reduce((a, r) => a + (r.event ? 1 : 0), 0);
      if (rows.length < minN || nEvents < minEvents) continue;
      const m = st.mean(rows.map((r) => r.covariates[0]));
      const s = st.sd(rows.map((r) => r.covariates[0]));
      if (!Number.isFinite(s) || s <= 0) continue;
      for (const r of rows) r.covariates[0] = (r.covariates[0] - m) / s;
      const fit = coxph(rows, [gene]);
      if (!fit || (requireConverged && !fit.converged)) continue;
      out.push({
        Gene: gene,
        HR: fit.HR[0],
        HR_lower: fit.hrLower[0],
        HR_upper: fit.hrUpper[0],
        p_value: fit.p[0],
        p_signif: fit.p[0] < 0.05 ? '*' : '',
        n: fit.n, nEvents: fit.nEvents, converged: fit.converged, iters: fit.iters
      });
    }
    return out;
  };
  ...
  export function multivariate(time, event, genes, exprRows) {
    const rows = [];
    for (let i = 0; i < time.length; i++) {
      if (!isFinite(time[i]) || time[i] === null) continue;
      if (time[i] <= 0) continue;
