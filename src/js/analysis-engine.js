...
// Modo de compatibilidade com o Script.R do projeto. Diferente da coorte basal,
// preserva todas as amostras do perfil de expressão e replica o alinhamento
// sample_id -> patient_id usado no R antes de Kaplan-Meier/Cox.
export function buildRCompatibleReferenceVectors(dp){
  ...
  return{sampleIndices,time,event,sampleIds:selected.map(x=>x.sampleId),patients:selected.map(x=>x.patientId),clinicalRows:selected.map(x=>x.clinical),endpointLabel:endpointAdequate?'Sobrevida Global (OS) · compatibilidade R':'Sobrevida Global (OS) · compatibilidade R — insuficiente',endpointKey:'OS',endpointTimeColumn:osTimeCol,endpointEventColumn:osEventCol,nEvents,expressionAligned:sampleIds.length>0,endpointAdequate,endpointPolicy:'OS_ONLY_R_COMPAT',mode:'r-compatible'};
}
...
export function rCompatibleExpressionValues(dp,gene){
  ...
  // O Script.R imputa NA pela mediana do gene ANTES do alinhamento de sobrevida.
...
export async function analyzeAgainstStudy(dados,dp){
  ...
  for(const geneRaw of dados.biomarcadores||[]){
    const gene=String(geneRaw).toUpperCase(),rawPatientExpr=num(dados.expressao?.[gene]),exprRow=expMap.get(gene)||null,deaRef=deaMap.get(gene)||null;let cohortMedian=NaN,q25=NaN,q75=NaN,patientGroup=null,referencePosition=null,percentile=NaN,km=null,cox=null;const patientExpr=transformExpressionValue(rawPatientExpr,dp.pack);
    if(exprRow&&vectors.sampleIndices.length>=10){const cohortValues=transformedValues(vectors,exprRow,dp.pack).filter(Number.isFinite);cohortMedian=median(cohortValues);q25=quantile(cohortValues,.25);q75=quantile(cohortValues,.75);percentile=percentileRank(cohortValues,patientExpr);km=survivalForGene(vectors,exprRow,dp.pack);cox=coxForGene(vectors,exprRow,gene,dp.pack);if(Number.isFinite(patientExpr)&&Number.isFinite(cohortMedian))patientGroup=patientExpr>=cohortMedian?'Alto':'Baixo';if(Number.isFinite(patientExpr)&&Number.isFinite(q25)&&Number.isFinite(q75))referencePosition=patientExpr<q25?'Abaixo da faixa central':patientExpr>q75?'Acima da faixa central':'Dentro da faixa central';if(Number.isFinite(rawPatientExpr)&&Number.isFinite(cohortMedian))chartExpression.push({gene,patientRaw:rawPatientExpr,patient:patientExpr,median:cohortMedian,q25,q75,referencePosition,percentile});if(km)kmResults.push({gene,patientGroup,referencePosition,endpoint:vectors.endpointLabel,...kmChartData(km)});if(cox)coxResults.push(cox);}
