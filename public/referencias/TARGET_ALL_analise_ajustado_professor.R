...
# ---- Heatmap — Top 40 DEGs (PNG alta resolução) + Top 15 DEGs (PNG separado) ----
#
# AJUSTE 1: quando poucos genes passam no filtro de significância
# (< 5), o bloco inteiro era pulado ANTES sem nenhuma mensagem — por
# isso "rodava bonitinho" mas nenhum PNG de heatmap aparecia. Agora
# há um aviso explícito nesse caso (branch 'else' abaixo).
#
# AJUSTE 2: o segundo pheatmap() (top 15) antes só desenhava no
# dispositivo gráfico ativo e NUNCA era salvo em arquivo — por isso,
# rodando via Rscript/modo batch (sem janela gráfica), ele
# simplesmente não aparecia em lugar nenhum. Agora ele é salvo em
# fig5_heatmap_top15.png, dentro de seu próprio png()/dev.off().
...
if (length(top_deg_genes) >= 5) {
  ...
  # salvar_heatmap(): fecha QUALQUER dispositivo gráfico pendente antes
  # de abrir o png() (evita o bug do Windows em que um device "órfão"
  # de uma execução anterior faz o dev.off() fechar o device errado,
  # gerando um PNG de 0 bytes mesmo sem erro nenhum). tryCatch garante
  # que o dev.off() SEMPRE rode, mesmo se pheatmap() falhar por dentro.
  salvar_heatmap <- function(caminho, mat, titulo, largura, altura, fontsize_row) {
    while (!is.null(dev.list())) dev.off()   # limpa devices órfãos
    grDevices::png(caminho, width = largura, height = altura, res = 300)
    resultado <- tryCatch({
      pheatmap(
        mat,
        main              = titulo,
        ...
        show_colnames     = FALSE, fontsize_row  = fontsize_row,
        border_color      = NA
      )
      TRUE
    }, error = function(e) {
      message("  ERRO ao desenhar '", caminho, "': ", e$message)
      FALSE
    })
    dev.off()
    tamanho <- file.size(caminho)
    if (isTRUE(resultado) && !is.na(tamanho) && tamanho > 0) {
      message(basename(caminho), " salvo (", tamanho, " bytes).")
    } else {
      message("AVISO: ", basename(caminho),
              " foi criado mas ficou vazio/corrompido (", tamanho,
              " bytes). Verifique dev.list() e se há outro processo/",
              "antivírus bloqueando o arquivo.")
    }
  }

  salvar_heatmap(
    paste0(out, "fig4_heatmap_DEG.png"), heat_scaled,
    paste("Heatmap — Top", length(top_deg_genes), "DEGs (Relapse vs None)"),
    largura = 2800, altura = 2200, fontsize_row = 8
  )
  ...
  salvar_heatmap(
    paste0(out, "fig5_heatmap_top15.png"), heat15,
    "Heatmap — Top 15 DEGs (Relapse vs None)",
    largura = 2200, altura = 1800, fontsize_row = 11
  )

} else {
  message("AVISO: heatmap NÃO gerado — apenas ", length(top_deg_genes),
          " genes DEG passaram no filtro de significância (mínimo exigido: 5).")
  message("  Verifique 'Genes DEG (adj.P<0.05)' impresso acima.")
  message("  Se o número for baixo, considere afrouxar o critério apenas para",
          " fins de visualização, ex.: filter(adj.P.Val < 0.10) ou",
          " abs(logFC) > 0.3, sem alterar a tabela de resultados salva em CSV.")
}
